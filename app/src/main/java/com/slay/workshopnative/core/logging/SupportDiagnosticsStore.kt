...
class SupportDiagnosticsStore @Inject constructor() {
    ...
    fun recordRecoveryEvent(action: String, fields: Map<String, String?> = emptyMap()) {
        synchronized(lock) {
            recoveryTimeline +=
                SupportStructuredEvent(
                    timestampMs = System.currentTimeMillis(),
    ...
    fun recordDownloadDecision(snapshot: SupportDownloadDecisionSnapshot) {
        synchronized(lock) { downloadDecisions[snapshot.taskId] = snapshot }
    }

    fun recordPerformanceSnapshot(snapshot: SupportPerformanceSnapshot) {
        synchronized(lock) { performanceSnapshots[snapshot.taskId] = snapshot }
    }
    ...
    fun recordCdnTaskPlan(
        ...
        synchronized(lock) {
            ...
            if (existing == null) {
                cdnTaskSummaries[taskId] =
                    MutableCdnTaskSummary(
                        timestampMs = System.currentTimeMillis(),
    ...
    fun recordCdnFailure(taskId: String?, host: String?, message: String?) {
        if (host.isNullOrBlank()) return
        synchronized(lock) {
    ...
    fun incrementCounter(category: String, host: String? = null, amount: Int = 1) {
    ...
    fun snapshot(): SupportDiagnosticsSnapshot =
        synchronized(lock) {
    ...
    private fun httpCategoryFor(rawUrl: String): String {
        ...
        return when {
            host == "steamcommunity.com" && path == "/sharedfiles/ajaxgetworkshops/" ->
                "explore_page_http"
            host == "steamcommunity.com" && path == "/workshop/ajaxfindworkshops/" ->
                "explore_search_http"
            host == "steamcommunity.com" && path == "/workshop/browse/" -> "workshop_browse_http"
            host == "steamcommunity.com" && path == "/sharedfiles/filedetails/" ->
                "workshop_detail_page_http"
            ...
            host == "api.steampowered.com" && path.contains("getpublishedfiledetails") ->
                "published_file_details_http"
