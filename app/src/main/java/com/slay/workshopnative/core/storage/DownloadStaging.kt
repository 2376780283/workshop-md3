...
fun directDownloadStagingFile(context: Context, taskId: String, fileName: String): File {
    val directory =
        File(context.filesDir, "$STAGING_ROOT_DIR_NAME/direct/$taskId").apply { mkdirs() }
    return File(directory, sanitizeFileName(fileName, "download.bin"))
}

fun workshopDownloadStagingRoot(context: Context, taskId: String, rootName: String): File {
    return File(
            context.filesDir,
...
fun clearDownloadStaging(context: Context, taskId: String) {
