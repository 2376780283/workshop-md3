...
data class PlannedWorkshopExportFile(val source: File, val relativePath: String)

data class WorkshopExportPlan(val files: List<PlannedWorkshopExportFile>, val renamedCount: Int)
...
fun buildWorkshopExportPlan(localRootDir: File): WorkshopExportPlan {
    ...
    val plannedFiles =
        sourceFiles.mapIndexed { index, source ->
            val sourceSegments =
                source
                    .relativeTo(localRootDir)
                    .invariantSeparatorsPath
                    .split('/')
                    .filter(String::isNotBlank)
            val originalSegments =
                sourceSegments.mapIndexed { segmentIndex, segment ->
                    sanitizeFileName(
                        raw = segment,
                        fallback =
                            if (segmentIndex == sourceSegments.lastIndex) "file" else "folder",
                    )
                }
            val resolvedPath =
            ...
            resolvedSegments
                .dropLast(1)
                .runningReduceOrNull { acc, segment -> "$acc/$segment" }
                ?.let(exportedDirectoryPaths::addAll)
            PlannedWorkshopExportFile(source = source, relativePath = resolvedPath)
        }

    return WorkshopExportPlan(files = plannedFiles, renamedCount = renamedCount)
}

fun appendTimestampSuffix(name: String, timestampMs: Long = System.currentTimeMillis()): String {
...
fun appendOrdinalSuffix(name: String, ordinal: Int): String {
    val dot = name.lastIndexOf('.')
    val base = if (dot > 0) name.substring(0, dot) else name
    val extension = if (dot > 0) name.substring(dot) else ""
...
private fun resolveUniqueRelativePath(
    ...
    while (true) {
        val prefixConflict =
            (1 until segments.size).firstOrNull { prefixLength ->
...
private fun appendSegmentSuffix(baseName: String, ordinal: Int, isLeaf: Boolean): String {
    return if (isLeaf) {
