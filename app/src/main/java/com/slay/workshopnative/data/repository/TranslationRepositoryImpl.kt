...
constructor(
    ...
    @Serializable private data class AzureTranslateBody(val text: String)

    @Serializable private data class AzureDetectedLanguage(val language: String? = null)

    @Serializable private data class AzureTranslationEntry(val text: String? = null)

    @Serializable
    private data class AzureTranslationResponseItem(
        @SerialName("detectedLanguage") val detectedLanguage: AzureDetectedLanguage? = null,
        val translations: List<AzureTranslationEntry> = emptyList(),
    )

    @Serializable private data class AzureErrorEnvelope(val error: AzureErrorPayload? = null)

    @Serializable
    private data class AzureErrorPayload(val code: String? = null, val message: String? = null)

    @Serializable
    private data class GoogleTranslationResponse(val data: GoogleTranslationData? = null)
    ...
    private data class GoogleWebTranslationResponse(
        ...
        @SerialName("src") val detectedSourceLanguage: String? = null,
    )

    @Serializable
    private data class GoogleWebSentence(@SerialName("trans") val translatedText: String? = null)

    @Serializable private data class GoogleErrorEnvelope(val error: GoogleErrorPayload? = null)
    ...
    @Volatile private var microsoftEdgeAccessToken: String? = null
    @Volatile private var microsoftEdgeAccessTokenExpireAtMs: Long = 0L
            ...
            val config =
                preferencesStore.snapshot().let { prefs ->
                    EffectiveTranslationConfig(
                        ...
                        azureEndpoint =
                            prefs.translationAzureEndpoint.ifBlank {
    ...
    private fun translateWithAzure(
        ...
        val requestBody =
            json.encodeToString(chunks.map(::AzureTranslateBody)).toRequestBody(JSON_MEDIA_TYPE)
        val url =
            "$endpoint/translate"
                .let { baseUrl ->
                    baseUrl
                        ...
                        ?.build() ?: error("Azure Translator endpoint 无效")
                }
        val requestBuilder =
            Request.Builder()
                .url(url)
        ...
        val payload =
            okHttpClient.newCall(requestBuilder.build()).execute().use { response ->
                ...
                check(response.isSuccessful) { buildAzureErrorMessage(response.code, payload) }
                payload
            }
        ...
        val translatedText =
            items
                .map { item -> decodeHtmlEntities(item.translations.firstOrNull()?.text.orEmpty()) }
                .joinToString("\n\n")
                .trim()
    ...
    private fun translateWithGoogle(
        ...
        val formBodyBuilder = FormBody.Builder().add("target", "zh-CN").add("format", TEXT_FORMAT)
        chunks.forEach { chunk -> formBodyBuilder.add("q", chunk) }
        val url =
            GOOGLE_TRANSLATE_URL.toHttpUrlOrNull()
                ?.newBuilder()
                ?.addQueryParameter("key", config.googleApiKey)
                ?.build() ?: error("Google Cloud Translation endpoint 无效")
        val request = Request.Builder().url(url).post(formBodyBuilder.build()).build()
        val payload =
            okHttpClient.newCall(request).execute().use { response ->
                val payload = response.body?.string().orEmpty()
                check(response.isSuccessful) { buildGoogleErrorMessage(response.code, payload) }
                payload
            }
        ...
        val translatedText =
            translations
                .map { entry -> decodeHtmlEntities(entry.translatedText.orEmpty()) }
                .joinToString("\n\n")
                .trim()
    ...
    private fun translateWithGoogleWeb(chunks: List<String>): TranslationResult {
        val responses =
            chunks.map { chunk ->
                val url =
                    GOOGLE_WEB_TRANSLATE_URL.toHttpUrlOrNull()
                        ?.newBuilder()
                        ...
                        ?.build() ?: error("Google Web 翻译地址无效")
                val request =
                    Request.Builder().url(url).get().header("Referer", GOOGLE_WEB_REFERER).build()
                val payload =
                    okHttpClient.newCall(request).execute().use { response ->
                        val payload = response.body?.string().orEmpty()
    ...
    private fun translateWithMicrosoftEdge(chunks: List<String>): TranslationResult {
        val requestBody =
            json.encodeToString(chunks.map(::AzureTranslateBody)).toRequestBody(JSON_MEDIA_TYPE)
        val url =
            DEFAULT_AZURE_TRANSLATOR_ENDPOINT.removeSuffix("/").let { baseUrl ->
                "$baseUrl/translate"
                    .toHttpUrlOrNull()
                    ?.newBuilder()
                    ?.addQueryParameter("api-version", AZURE_TRANSLATE_API_VERSION)
                    ?.addQueryParameter("to", TARGET_LANGUAGE)
                    ?.build() ?: error("Microsoft Edge 翻译地址无效")
            }
        val request =
            Request.Builder()
                .url(url)
    ...
    private fun buildCacheKey(config: EffectiveTranslationConfig, normalizedText: String): String {
    ...
    private fun getMicrosoftEdgeAccessToken(): String {
        ...
        microsoftEdgeAccessToken
            ?.takeIf { now < microsoftEdgeAccessTokenExpireAtMs }
            ?.let { token ->
                return token
            }
        ...
        synchronized(microsoftEdgeTokenLock) {
            ...
            microsoftEdgeAccessToken
                ?.takeIf { refreshedNow < microsoftEdgeAccessTokenExpireAtMs }
                ?.let { token ->
                    return token
                }

            val request = Request.Builder().url(MICROSOFT_EDGE_AUTH_URL).get().build()
            val token =
                okHttpClient.newCall(request).execute().use { response ->
                    ...
                    check(payload.count { it == '.' } == 2) { "Microsoft Edge 翻译认证失败，未拿到有效令牌" }
                    payload
                }
            microsoftEdgeAccessToken = token
            microsoftEdgeAccessTokenExpireAtMs =
                System.currentTimeMillis() + MICROSOFT_EDGE_ACCESS_TOKEN_TTL_MS
            return token
        }
    }

    private fun buildAzureErrorMessage(statusCode: Int, payload: String): String {
        val remoteMessage =
            runCatching { json.decodeFromString<AzureErrorEnvelope>(payload).error?.message }
                .getOrNull()
                ?.trim()
                .orEmpty()
        ...
        return when {
            ...
            statusCode == 429 -> "Azure Translator 配额已用尽或请求过于频繁，请稍后再试"
    ...
    private fun buildGoogleErrorMessage(statusCode: Int, payload: String): String {
        val remoteMessage =
            runCatching { json.decodeFromString<GoogleErrorEnvelope>(payload).error?.message }
                .getOrNull()
                ?.trim()
                .orEmpty()
        ...
        return when {
            ...
            statusCode == 403 -> "Google Cloud Translation 无权访问，请检查 API 是否启用、Key 限制和项目结算状态"
            statusCode == 429 -> "Google Cloud Translation 配额已用尽或请求过于频繁，请稍后再试"
            remoteMessage.isNotBlank() -> remoteMessage
            else -> "Google Cloud Translation 请求失败：HTTP $statusCode"
        }
    }

    private fun buildGoogleWebErrorMessage(statusCode: Int, payload: String): String {
        return when {
            statusCode == 403 -> "Google Web 翻译当前不可用，可能被服务端限制或需要更换网络环境"
            statusCode == 429 -> "Google Web 翻译请求过于频繁，请稍后再试"
            payload.isNotBlank() && payload.length < 120 -> payload
            else -> "Google Web 翻译请求失败：HTTP $statusCode"
        }
    }

    private fun buildMicrosoftEdgeAuthErrorMessage(statusCode: Int, payload: String): String {
        return when {
            statusCode == 403 || statusCode == 429 -> "Microsoft Edge 翻译认证接口当前不可用，可能被服务端限制"
            payload.isNotBlank() && payload.length < 120 -> payload
            else -> "Microsoft Edge 翻译认证失败：HTTP $statusCode"
        }
    }

    private fun buildMicrosoftEdgeTranslateErrorMessage(statusCode: Int, payload: String): String {
        val remoteMessage =
            runCatching { json.decodeFromString<AzureErrorEnvelope>(payload).error?.message }
                .getOrNull()
                ?.trim()
        ...
        return when {
            statusCode == 401 || statusCode == 403 -> "Microsoft Edge 翻译通道认证失败，可能已被微软调整或暂时不可用"
            statusCode == 429 -> "Microsoft Edge 翻译请求过于频繁，请稍后再试"
            remoteMessage.isNotBlank() -> remoteMessage
