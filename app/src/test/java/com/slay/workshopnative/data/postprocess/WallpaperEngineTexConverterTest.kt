...
class WallpaperEngineTexConverterTest {
    ...
    fun convertsR8TexToGrayscalePng() {
        val texBytes =
            buildTex(formatId = 9, width = 2, height = 1, payload = byteArrayOf(0x11, 0x7F))
    ...
    fun passesThroughRawPngPayload() {
        val pngBytes =
            encodeReferencePng(width = 1, height = 1, rgba = byteArrayOf(0x12, 0x34, 0x56, 0x78))
        val texBytes = buildTex(formatId = 0, width = 1, height = 1, payload = pngBytes)
    ...
    fun rawMp4FastPathInspectorRecognizesAndCopiesPayload() {
        val mp4Bytes =
            byteArrayOf(0x00, 0x00, 0x00, 0x18, 0x66, 0x74, 0x79, 0x70, 0x6D, 0x70, 0x34, 0x32)
        val texBytes = buildTex(formatId = 0, width = 1, height = 1, payload = mp4Bytes)
    ...
    private fun buildTex(formatId: Int, width: Int, height: Int, payload: ByteArray): ByteArray {
    ...
    private fun decodeSimpleRgbaPng(bytes: ByteArray, width: Int, height: Int): List<Int> {
        ...
        repeat(height) {
            ...
            repeat(width * 4) { rgba += scanlines[cursor++].toInt() and 0xFF }
        }
        return rgba
    }

    private fun encodeReferencePng(width: Int, height: Int, rgba: ByteArray): ByteArray {
